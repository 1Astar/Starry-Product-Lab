  ...
  return (
    <div className="welcome-panel">
      ...
      <div className="welcome-copy">
        ...
        <p className="universe-note">这里是我的产品宇宙：<br />有正在生长的项目，也有被暂时停靠的灵感。</p>
  ...
  return (
    <div className="project-universe">
      ...
      <div className="star-map">
        <div className="orbit-line orbit-one" /><div className="orbit-line orbit-two" /><div className="orbit-line orbit-three" />
        <section className="star-layer public-layer" aria-label="公开作品">
          <h3>公开作品</h3>
          {publicWorks.map((project, index) => (
            <ProjectPlanet project={project} index={index} key={project.id} variant="public" onOpen={onOpenProject} />
          ))}
        </section>
        <section className="star-layer case-layer" aria-label="工作案例">
          <h3>工作案例</h3>
          {workCases.map((project, index) => (
            <ProjectPlanet project={project} index={index} key={project.id} variant="case" onOpen={onOpenProject} />
          ))}
        </section>
        <section className="star-layer idea-layer" aria-label="灵感实验">
          <h3>灵感实验</h3>
          {ideas.map((idea, index) => (
            <motion.button
              className={`idea-signal idea-${index + 1}`}
              key={idea.title}
              type="button"
              whileHover={{ y: -4, scale: 1.03 }}
              onClick={onOpenIdeas}
            >
              <Sparkles size={13} /><span>{idea.title}</span><small>{idea.status}</small>
            </motion.button>
          ))}
        </section>
      </div>
  ...
  return (
    <div className="project-file">
      ...
      <div className="file-cover">
        ...
        <div className={`file-image crop-${project.id}`} />
      </div>
