...
export function ProjectGalaxy3D({
  ...
  const nodes = useMemo<GalaxyNode[]>(() => {
    ...
    const projectNodes: GalaxyNode[] = [
      { id: "follow-heart", label: "随心而行", meta: "今日主线", status: "P1 结果页优化", note: byId.get("follow-heart")?.humanNote ?? "", kind: "public", x: -0.18, y: 0.02, size: 0.82, project: byId.get("follow-heart") },
      { id: "competitor-workbench", label: "竞品分析工作台", meta: "观察与分析", status: "进行中", note: byId.get("competitor-workbench")?.humanNote ?? "", kind: "public", x: -2.55, y: 0.72, size: 0.44, project: byId.get("competitor-workbench") },
      { id: "ai-companion", label: "AI Companion", meta: "陪伴星球", status: "长期迭代", note: byId.get("ai-companion")?.humanNote ?? "", kind: "public", x: 0.35, y: 1.42, size: 0.5, project: byId.get("ai-companion") },
      { id: "star-pm", label: "Star PM", meta: "轨道控制", status: "概念验证", note: byId.get("star-pm")?.humanNote ?? "", kind: "public", x: 2.35, y: -0.45, size: 0.48, project: byId.get("star-pm") },
      { id: "ai-pet-hardware", label: "AI 宠物", meta: "脱敏卫星", status: "工作案例", note: byId.get("ai-pet-hardware")?.humanNote ?? "", kind: "case", x: -2.62, y: -1.22, size: 0.35, project: byId.get("ai-pet-hardware") },
      { id: "iot-ops", label: "IoT 远程运维平台", meta: "连接与控制", status: "流程案例", note: byId.get("iot-ops")?.humanNote ?? "", kind: "case", x: 0.18, y: -1.88, size: 0.35, project: byId.get("iot-ops") },
      { id: "yuanjing-miniapp", label: "元井小程序", meta: "暂时停靠", status: "公开入口", note: byId.get("yuanjing-miniapp")?.humanNote ?? "", kind: "case", x: 2.42, y: -1.48, size: 0.32, project: byId.get("yuanjing-miniapp") },
    ...
    <div className="galaxy-3d">
      ...
      {nodes.map((node) => (
        <motion.button
          ...
          whileHover={{ y: -7, rotate: node.kind === "idea" ? -2 : 1.5, scale: 1.035 }}
